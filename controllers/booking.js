  ...
  const { date } = req.body
    ...
    try {    
     ...
     const resp = await connection.query(
       `SELECT booking.id, DATE_FORMAT(booking.time_start,'%Y-%m-%d %H:%i') AS "timeStart", booking.table_id, booking.time_finish, booking.client_id, booking.order_client, booking.status, table_room.name, table_room.number, clients.name AS "client_name", clients.phone, clients.email FROM booking RIGHT JOIN table_room ON booking.table_id = table_room.id LEFT JOIN clients ON booking.client_id = clients.id 
...
const addBooking = async (req, res) => {  
  const { tableId, timeStart, timeFinish, clientId, orderClient, status, phone, userName } = req.body
  ...
  const  timeStartForFree = timeStart.split(" ")
    ...
    const resp = await connection.execute(
      ...
      [null, tableId, timeStart, null, addUserOnTime[0].insertId, orderClient, status]);        
      res.status(200).json({data: resp[0]})
...
const addBookingForAdmin = async (req, res) => {  
  ...
  try {
    ...
    res.status(500).json({ status: "error", message: "Не удалось внести изменение2" });
  }
};
