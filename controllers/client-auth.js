...
const authUser = async (req, res) => {
	const { login, password, tokens } = req.body
	...
	if (tokens) {
		const isUserForToken = await connection.query(
			`SELECT * FROM firebase   
       WHERE id_client = ?`,
			[isUser[0][0].id]
		)

		if (isUserForToken[0].length > 0) {
			const resp = await connection.execute(`UPDATE firebase SET token = ?`, [
				tokens,
			])
		} else {
			const resp = await connection.execute(
				`INSERT INTO firebase(id, token, id_client) VALUE (?, ?, ?)`,
				[null, tokens, isUser[0][0].id]
			)
		}
	}
	...
	res
		...
		.json({
			...
			user: {
				id: isUser[0][0].id,
				name: isUser[0][0].name,
				email: isUser[0][0].email,
				phone: isUser[0][0].phone,
			},
		})
}
